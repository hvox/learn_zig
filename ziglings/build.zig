...
pub fn build(b: *Builder) void {
    ...
    for (exercises) |ex| {
        ...
        const file_path = std.fs.path.join(b.allocator, &[_][]const u8{
            if (use_healed) "patches/healed" else "./", ex.main_file,
        }) catch unreachable;
...
const ZiglingStep = struct {
    ...
    fn doCompile(self: *@This()) ![]const u8 {
        ...
        const zig_file = std.fs.path.join(builder.allocator, &[_][]const u8{ if (self.use_healed) "patches/healed" else "./", self.exercise.main_file }) catch unreachable;
